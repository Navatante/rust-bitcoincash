...
//!
//! Support for legacy addresses (base58 encoding only).
//! BCH uses P2PKH and P2SH addresses. No SegWit, no bech32, no Taproot.
//!
//! # Examples
//!
...
use crate::script::{
    RedeemScriptSizeError, Script, ScriptExt as _, ScriptHash, ScriptHashableTag, ScriptPubKey,
    ScriptPubKeyBuf, ScriptPubKeyBufExt as _, ScriptPubKeyExt as _,
};

#[rustfmt::skip]                // Keep public re-exports separate.
#[doc(inline)]
pub use self::error::{
        Base58Error, FromScriptError, InvalidBase58PayloadLengthError,
        InvalidLegacyPrefixError, LegacyAddressTooLongError, NetworkValidationError,
        ParseError, UnknownAddressTypeError,
};
        ...
        D: serde::de::Deserializer<'de>,
    {
            ...
            Address<U>: FromStr,
        {
            ...
            fn expecting(&self, f: &mut Formatter) -> core::fmt::Result {
                f.write_str("A Bitcoin Cash address")
            }
...
impl Address {
    ...
    ///
    /// This is the preferred address type for Bitcoin Cash.
    #[inline]
    ...
    ///
    /// This address type was introduced with BIP-0016 and is used for multi-sig and other
    /// complex scripts on Bitcoin Cash.
    #[inline]
    ...
    /// Checks whether or not the address is following Bitcoin Cash standardness rules when
...
impl Address<NetworkUnchecked> {
    ...
    /// parsing, such addresses are always assumed to be testnet addresses. So if one wants to
    /// check if an address belongs to a certain network a simple comparison is not enough anymore.
...
///
/// Only legacy base58 addresses prefixed with `1`, `2`, `3`, `m` or `n` are supported.
///
/// # Errors
///
...
impl<U: NetworkValidationUnchecked> FromStr for Address<U> {
    ...
    fn from_str(s: &str) -> Result<Self, ParseError> {
        if ["1", "2", "3", "m", "n"].iter().any(|&prefix| s.starts_with(prefix)) {
            let address = Address::from_base58_str(s)?;
            Ok(Address::from_inner(address.into_inner()))
        } else {
            Err(Base58Error::InvalidLegacyPrefix(InvalidLegacyPrefixError {
                invalid: s.as_bytes().first().copied().unwrap_or(0),
            })
            .into())
        }
    }
...
/// Used in `is_related_to_pubkey` for P2SH-wrapped compatibility checks.
...
mod tests {
    ...
    use crate::script::{RedeemScriptBuf, ScriptBufExt as _};
